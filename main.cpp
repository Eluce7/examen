...
int main()
{
    ...
    enum eleccionOpLibros { INSERTAR_LIBROS = 1, BUSCAR_LIBROS, ELIMINAR_LIBROS, MOSTRAR_LIBROS, GUARDAR_LIBROS, FILTRAR_LIBROS, SALIR_LIBROS };
    ...
    auto operarLibros = [&menuOpLibros, &operarAutores, listaLibros, listaAutores](int eleccion)
        {
            ...
            case INSERTAR_LIBROS:
            {
                ...
                do
                {
                    ...
                    if (punteroAutor)
                    {
                        listaLibros->insertarACola(Libro(id, titulo, punteroAutor, Fecha(0,0,year)));
